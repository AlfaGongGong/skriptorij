...
async def _scoruj_kvalitetu(
    ...
    try:
        ...
        if self_obj is not None:
            try:
                # merged: get_scorer_sys_override je sada u ovom fajlu
...
# ─────────────────────────────────────────────────────────────
# SCORER TRACKER — preuzeto iz quality_scorer_v2.py
# ─────────────────────────────────────────────────────────────
...
class TranslatorTracker:
    ...
    def dohvati(self, file_name: str, chunk_idx: int):
        stem = f"{file_name}_blok_{chunk_idx}"
        with self._lock:
...
def _odaberi_scorer_provider(prevodilac_provider: str):
...
def get_scorer_sys_override(self_obj, prevodilac_provider, chunk_idx, file_name):
