...
# Fajlovi koji nisu proza — ne ocjenjuju se standardnim scorerom
_NON_PROZA_PATTERNS = [
    "acknowledgement", "zahvale", "copyright", "author", "autor",
    "colophon", "imprint", "dedication", "posveta", "foreword",
    "preface", "predgovor", "toc", "sadrzaj", "contents",
    "about", "biography", "bibliografija", "bibliography",
]

def _je_non_proza(naziv_fajla: str) -> bool:
    naziv = naziv_fajla.lower()
    return any(p in naziv for p in _NON_PROZA_PATTERNS)
...
_BASE_DIR = Path("/storage/emulated/0/termux/Skriptorij")
_LOGS_DIR = _BASE_DIR / "logs"
_CHK_DIR = _BASE_DIR / "checkpoints"
...
_EKAVIZMI = [
    ...
    r"\bsređuje\b",
    r"\bkree\b",
    r"\bkrenuo sam\b",  # krenuo = ok, ali kree nije
    r"\bmeđutim\b",  # medjutim je ekavski oblik bez đ
    r"\bmoze\b",
    r"\bmogu[ćc]e\b(?<!\bmoguće\b)",
    r"\bvidi se\b",
        ...
        for chk_file in sorted(dir_path.glob("*.chk")):
            try:
                ...
                if len(tekst.strip()) >= MIN_DULJINA_BLOKA:
                    ...
                    idx = (
                        int(re.search(r"(\d+)", chk_file.stem).group(1))
                        if re.search(r"(\d+)", chk_file.stem)
                        else 0
                    )
                    chunkovi.append(
                        {
...
class QABenchmark:
    ...
    def __init__(self):
        ...
        if self._hunspell_ok:
            logger.info(
                "qa_benchmark: hunspell dostupan — koristim za provjeru pravopisa"
            )
        else:
    ...
    def analiziraj_fajl(
        ...
        n = len(rezultati)
        avg_benchmark = (
            round(sum(r["benchmark_score"] for r in rezultati) / n, 2) if n else 0.0
        )
        avg_quality = None
        q_vals = [
            r["quality_score"] for r in rezultati if r["quality_score"] is not None
        ]
        if q_vals:
        ...
        try:
            with open(trend_path, "w", encoding="utf-8") as f:
                json.dump(
                    {**baseline_payload, "trend": trend_report},
                    f,
                    ensure_ascii=False,
                    indent=2,
                )
...
def _ispisi_summary(file_name: str, summary: dict, trend: dict) -> None:
    ...
    logger.info(
        ...
        + (
            f"  {promjene.get('avg_benchmark_score', {}).get('trend', '')}"
            if promjene
            else ""
        )
