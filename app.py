...
def create_app() -> Flask:
    ...
    def api_fleet():
        try:
            raw = _get_fleet().get_fleet_ui()
            # Normalizacija — renderFleet očekuje {PROV: {total, success_rate, keys: [...]}}
            # get_fleet_ui() vraća upravo taj format, ali dodajemo obranu od None/praznog
            if not raw or not isinstance(raw, dict):
                return jsonify({})
            return jsonify(raw)
        except Exception as e:
            logger.exception("[api_fleet] Greška pri dohvaćanju flote")
            # Vraćamo prazan dict, NE {"error": "..."} — renderFleet ne zna obraditi error key
            return jsonify({}), 200
    ...
    try:
        ...
        for line in repl_content.splitlines():
            ...
            if sep:
                ...
                if len(parts) == 2:
                    orig, repl = parts[0].strip(), parts[1].strip()
        ...
        applied = 0
        files_changed = 0
        for chk_file in chk_dir.glob('*.chk'):
            try:
                txt = chk_file.read_text(encoding='utf-8', errors='ignore')
                original = txt
        ...
        return jsonify({'ok': True, 'applied': applied, 'files': files_changed})
