...
_MODELS_ENDPOINTS: dict[str, str] = {
    ...
    # Native endpoint vraća supportedGenerationMethods — tačnija detekcija dostupnih modela
...
def _fetch_gemini_native_models(api_key: str, timeout: float = 10.0) -> list[str]:
    """
    Dohvaća listu Gemini modela putem native /v1beta/models endpointa.

    Native endpoint koristi ?key= auth (ne Bearer) i vraća:
      {"models": [{"name": "models/gemini-2.5-flash", "supportedGenerationMethods": [...], ...}]}

    Filtriramo samo modele koji:
      - podržavaju generateContent (chat/completions sposobni)
      - sadrže "flash" u imenu (Free tier; Pro modeli su paid-only od aprila 2026)
      - ne sadrže "pro", "ultra", "embed", "tts", "imagen", "veo", "live"

    Sortiramo po _score_model_strength — najkorisniji model je prvi.
    """
    base_url = "https://generativelanguage.googleapis.com/v1beta/models"
    model_ids: list[str] = []
    page_token = None

    # Paginacija — Google vraća max 50 po stranici
    for _ in range(5):  # max 5 stranica = 250 modela
        ...
        for item in data.get("models", []):
            ...
            # Mora podržavati generateContent
            methods = item.get("supportedGenerationMethods", [])
            if "generateContent" not in methods:
                continue

            # Primijeni iste filtere kao i za ostale provajdere
    ...
    if not model_ids:
        logger.warning("[ModelDiscovery] GEMINI native /models — nema validnih modela, koristim fallback")
        return []

    model_ids.sort(key=lambda m: _score_model_strength("GEMINI", m), reverse=True)
    logger.info(
        "[ModelDiscovery] GEMINI native discovery: %d modela (best: %s)",
        len(model_ids), model_ids[0],
    )
    return model_ids
...
def fetch_models(provider: str, api_key: str, timeout: float = 10.0) -> list[str]:
    ...
    # ── GEMINI: native endpoint (?key= auth, drugačiji JSON format) ──────────
    if prov == "GEMINI":
