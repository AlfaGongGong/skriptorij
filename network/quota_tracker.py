# network/quota_tracker.py — OBNOVLJENO 2026-05-16
# Minimalna verzija: prati samo cooldown od Google-a, ne predviđa limite
    ...
    def record_error(self, status_code: int, retry_after: Optional[float] = None):
        with self._lock:
            ...
            if status_code in (401, 403):
                # Nevažeći ključ — permanentni blacklist (24h, jer nema smisla pokušavati)
                self._set_cooldown(86400.0, f"ključ nevažeći ({status_code})")
                logger.warning(
                    "[QuotaTracker] %s ...%s → %d — ključ blacklistiran na 24h",
                    self.provider, self.key[-4:], status_code,
                )
            elif status_code == 429:
                if retry_after and retry_after > 60:
                    # Dnevna kvota (RPD) — poštuj retry_after bez gornjeg capa od 20s
                    cooldown_s = min(float(retry_after), 86400.0)
                    reason = "RPD kvota" if retry_after > 3600 else "429 dugi cooldown"
...
class QuotaTracker:
    ...
    def _restore_cooldowns(self):
        """Vraća cooldown stanje s diska (perzistentno kroz restart)."""
        import json, os
        try:
                ...
                for key, info in keys.items():
                    ...
                    if until > now:
                        ...
                        if kq:
                            ...
                            restored += 1
                            logger.info(
                                "[QuotaTracker] Obnovljen cooldown %s ...%s: %.0fs (%s)",
                                prov, key[-4:], remaining, reason,
                            )
            if restored:
    ...
    def _persist_cooldowns(self):
        """Snima aktivne cooldowne na disk."""
        import json
        now = time.time()
        data = {}
        with self._lock:
    ...
    def record_response(self, provider: str, key: str, status_code: int, tokens: int = 0, retry_after=None, headers=None):
        pq = self._get_provider(provider.upper())
        kq = pq.get_key(key) if pq else None
        if not kq:
            return
        if status_code == 200:
            ...
            # Perzistuj cooldown na disk odmah nakon greške
